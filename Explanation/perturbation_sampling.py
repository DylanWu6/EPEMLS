...
def perturbation_pipeline(data, model, no_perturbations_per_sample, save_path, save_perturbations, delete,
    ...
    if save_perturbations:
        if type(data) is list:
            pkl.dump(all_perturbations,
                     open(os.path.join(save_path, 'perturbation_data_seed_{}.pkl'.format(seed)), 'wb'))
        elif type(data).__module__ == 'scipy.sparse.csr':
            sparse.save_npz(os.path.join(save_path, 'perturbation_data_seed_{}.npz'.format(seed)),
                            sparse.vstack(all_perturbations))
        else:
...
if __name__ == '__main__':
    ...
    # custom_objects_dict = {'custom_metric': custom_metrics}
