...
tf.config.optimizer.set_experimental_options({"mlir_v1": True})
...
def get_train_data_test_data(random_seed, binary_encoding=True):
    ...
    arr = np.genfromtxt(path_to_csv, dtype=float, delimiter=',', skip_header=1, usecols=columns_to_use)
...
def train_network(batch_size, epochs, loss, optimizer, vec_output, final_nonlinearity, random_seed):
    ...
    fname = 'models/model.{epoch:03d}--ACC_{val_accuracy:.4f}--.pb'
